    ...
    if data is None:
        ...
        for item in data:
            if item['current'] is not None:
                if 'startsAt' and item['current'] and item['current']['startsAt'] is not None and 'endsAt' and item['current'] and item['current']['endsAt'] is not None:
                ...
                if 'sidp' in item['current'] and item['current']['sidp'] is not None and len(item['current']['sidp']) > 1:
                    if int(item['current']['sidp']) in favourites:
                        ...
                        favourite = False
                    get_show_listitem(label, item['current']['sidp'], favourite, item['current']['channelSettings']['channelName'] + ' | ' + item['current']['title'] + ' | ' + title_time, url)
                else:
                    ...
                    list_item = xbmcgui.ListItem(label = item['current']['channelSettings']['channelName'] + ' | ' + title + ' | ' + title_time)
